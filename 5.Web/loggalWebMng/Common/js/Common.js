	...
	GetAjax: function (url, params, dateType, callback, basync, errorcallback, bloadingbar) {
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (dateType == "html" && result.indexOf("MoveAfterError") > 0) {
					$("body").html(result);
					...
					if (result != null && result != "") {
						try {
							...
							if (result.logout != undefined && result.logout == true) {
								$.MessageBox("세션이 만료되어 로그아웃처리 됩니다.").done(function () {
									BaseCommon.LoginInfo.doLogOut();
								});
							}
					...
					try {
						if (dateType == "html") {
							...
							if (result.indexOf("MoveAfterError") > 0) {
								$("body").html(result);
								$.MessageBox("세션이 만료되어 로그아웃처리 됩니다.").done(function () {
									BaseCommon.LoginInfo.doLogOut();
								});
...
var BaseCommon =
{
	...
	SetHtml: function (el, html) {
		...
		BaseCommon.errorinfo.imageNoimage();
		return newEl;
	},
	HostInfo: {
		Host: function () {
	...
	, FormInfo:
	{
		...
		, PostSubmit: function (url, arrData, target) {
			...
			try {
				$form.append('<input type="hidden" value="' + currentmenuName + '" name="currentmenuName">')
			} catch (e) { }
	...
	, StringInfo:  // String 관련 함수 정의
	{
		...
		, replaceAll: function (inputString, targetString, replacement) { //문자열 바꾸기
			...
			return inputString.split(targetString).join(replacement);
	...
	, Table: {
		tableCellCount : function (objName) {
	...
	/*에러 처리*/
	, errorinfo: {
		imageNoimage: function (obj) {
			if (obj != undefined) {
				$(obj).error(function () {
					$(this).attr("src", "/common/images/no-image.png");
				});
			} else {
				$("img").error(function () {
					$(this).attr("src", "/common/images/no-image.png");
...
var SNS = {
	Share: function (site, msg, url) {
		...
		switch (String(site).toUpperCase()) {
			case "TWITTER":
				// url = "//twitter.com/home?status=" + encodeURIComponent(msg) + " " + encodeURIComponent("//www.p2c.co.kr");
				url = "//twitter.com/home?status=" + encodeURIComponent(msg) + " " + encodeURIComponent(url);
				break;
			case "FACEBOOK":
				url = "//www.facebook.com/sharer.php?u=" + encodeURIComponent(msg) + " " + encodeURIComponent(url);
				break;
			case "GOOGLEPLUS":
				url = "//plus.google.com/share?url=" + encodeURIComponent(msg) + " " + encodeURIComponent(url);
				break;
			case "PINTEREST":
				url = "//www.pinterest.com/pin/create/button/?url=http%3A%2F%2Fwww.flickr.com%2Fphotos%2Fkentbrew%2F6851755809%2F&media=//p2c.co.kr/Common/images/main/main_img1.png&description=Next%20stop%3A%20Pinterest" + encodeURIComponent(msg) + " " + encodeURIComponent(url);
				break;
			default:
				url = "//www.facebook.com/sharer.php?u=" + encodeURIComponent(msg) + " " + encodeURIComponent(url);
				break;
